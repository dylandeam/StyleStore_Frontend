...
export class UploadService {
  ...
  uploadImage(file: File, folder: 'productos' | 'empleados' | 'clientes' | 'qr' | 'pagos' = 'productos'): Observable<UploadResponse> {
    const formData = new FormData();
    formData.append('file', file);
    return this.http.post<UploadResponse>(`${this.apiUrl}?folder=${folder}`, formData);
  }

  getFileUrl(url?: string | null, folder: 'productos' | 'empleados' | 'clientes' | 'qr' | 'pagos' = 'productos'): string {
    return this.getImageUrl(url, folder);
  }

  getImageUrl(url?: string | null, folder: 'productos' | 'empleados' | 'clientes' | 'qr' | 'pagos' = 'productos'): string {
